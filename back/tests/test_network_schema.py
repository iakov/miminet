...
class TestNetwork:
    ...
    def test_simple_network(self):
        ...
        h1 = Node(
            ...
            interfaces=[NodeInterface(connect="sw1", id="h1_1", name="h1-eth0", ip="192.168.1.10")],
        )

        h2 = Node(
            data=NodeData(id="h2", label="Host 2"),
            config=NodeConfig(type="host"),
            interfaces=[NodeInterface(connect="sw1", id="h2_1", name="h2-eth0", ip="192.168.1.11")],
        )
...
class TestNetworkValidation:
    ...
    def test_interfaces_reference_existing_nodes(self):
        ...
        _ = {n.data.id for n in network.nodes}
